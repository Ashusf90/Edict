...
export const runTool: EdictMcpTool = {
    ...
    handler: async ({ wasmBase64, limits, externalModules }) => {
        try {
            const result = await handleRun(wasmBase64, limits, externalModules);
            return {
                content: [
                    {
                        type: "text" as const,
