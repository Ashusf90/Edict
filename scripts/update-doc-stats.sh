...
# Example count (instant, safe with any filenames)
EXAMPLE_COUNT=$(find examples -maxdepth 1 -name '*.edict.json' -type f | wc -l | tr -d ' ')

# Test stats via vitest JSON reporter (written to temp file to avoid pipe buffering)
TMPFILE=$(mktemp)
...
if [ -z "$PYTHON" ]; then
  TEST_COUNT=""
  FILE_COUNT=""
elif ! npx vitest run --reporter=json > "$TMPFILE" 2>/dev/null; then
...
echo "Stats: ${TEST_COUNT:-?} tests across ${FILE_COUNT:-?} files, ${EXAMPLE_COUNT} examples"

# ---------------------------------------------------------------------------
# 2. Patch markdown files (content-based patterns, no line numbers)
# ---------------------------------------------------------------------------
...
for doc in "${CORE_DOCS[@]}"; do
  ...
  # Test stats (only if vitest succeeded)
